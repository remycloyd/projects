...
class circularList:  # fifo queue implementation using a singly linked list for storage.
    ...
    def dequeue(self):  # remove and return the first element of the list.
        ...
        if self.is_empty():          # special case: queue is now empty
            self._tail._next = self.head  # previous head was also tail
        return x
    ...
    def enqueue(self, e):
        ...
        if self.is_empty():
            ...
            self._tail._next = newest  # if queue not empty then new node is new tail: tail -> newest
    ...
    @staticmethod
    def _list_print(self, node):  # Print the  Linked list
...
slList.enqueue("2")
slList.enqueue("4")
slList.insertAtPos(0, "Z")
slList._list_print(slList, slList.head)
print("")
print(slList.head._next._next._next._next._element)
