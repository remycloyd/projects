...
public class KThread {
    private static final boolean ThreadQueue  = false;
	/**
	...
	if(CllrJoin == KThread.currentThread()){
    ...
    public void join() 
    {
    	...
    	if (sleeping || this.status == this.statusFinished ) 
    	{
    		return;
	}
	
	CllrJoin = KThread.currentThread();
	boolean intStatus = Machine.interrupt().disable(); 
	if (joinQueue == null) {
	...
	public void run() {
		...
		//new Alarm().waitUntil(40);
		Communicator cmct = new Communicator();
		cmct.speak(35);
    ...
    private static Lock locky = new Lock ();
    private static Condition2 condy = new Condition2(locky);
    ...
    private static KThread CllrJoin = null;
