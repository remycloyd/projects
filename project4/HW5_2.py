    ...
    for k in row:
        ...
        for m in k:
            ...
            if delt < ksub and delt <= ins and a == -1 or a == 1:  # 1
                row[ i ][ j ] = [ delt, "Delete " + s[ i - 1 ][ 0 ] + " by " + s[ i - 1 ][ 1 ], i - 1,
                                  j ]  # store delt score, what's being deleted (s[i-1]), [i-1], and [j]
            elif ksub <= delt and ksub <= ins and a == -1:  # 2
                if ksub > row[ i - 1 ][ j - 1 ][ 0 ]:
                    row[ i ][ j ] = [ ksub, "Sub " + t[ j - 1 ][ 0 ] + " by " + t[ j - 1 ][ 1 ] + " for " + s[ i - 1 ][
                        0 ] + " by " + s[ i - 1 ][ 1 ], i - 1,
                                      j - 1 ]  # store sub score, what's being subbed for what (t[j-1] for s[i-1]), and row[i-1][j-1]
                else:
                    row[ i ][ j ] = [ ksub, "Keep " + s[ i - 1 ][ 0 ] + " by " + s[ i - 1 ][ 1 ], i - 1,
                                      j - 1 ]  # store keep score, what's being kept (s[i-1]), and [i-1], [j-1]
            elif ins < delt and ins < ksub and a == -1 or a == 3:  # 3
                row[ i ][ j ] = [ ins, "Insert " + t[ j - 1 ][ 0 ] + " by " + t[ j - 1 ][ 1 ], i,
                                  j - 1 ]  # store ins score, what's being inserted t[j-1], i, and j
            elif a == 4:  # 4
                row[ i ][ j ] = [ 0, "", -1, -1 ]
    r = len(row) - 1
    c = len(row[ 0 ]) - 1
